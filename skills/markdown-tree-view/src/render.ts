...
export function renderDocument(
    markdown: string,
    { sourceName = "document.md", allowCloudflareAnalytics = false }: RenderOptions = {}
): string {
...
<summary class="tree-row"${node.anchor ? ` id="${escapeHtml(node.anchor)}"` : ""} tabindex="0"><span class="chevron" aria-hidden="true"></span><span class="heading-label" id="${escapeHtml(node.alias)}">${node.titleHtml || '<span class="empty-heading">（空标题）</span>'}</span><span class="source-location" title="源文件第 ${String(node.line)} 行">L${String(node.line)}</span></summary>
<div class="children">${renderBody(node.tokens, "section-body")}${node.children.map((child) => renderNode(child)).join("\n")}</div>
    ...
    // Hosting may inject the beacon; allowing its script alone would still block reports.
    const analyticsScript = allowCloudflareAnalytics ? " https://static.cloudflareinsights.com" : "";
    const connections = allowCloudflareAnalytics ? "'self' https://cloudflareinsights.com" : "'none'";
    return `<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
...
<meta name="referrer" content="${allowCloudflareAnalytics ? "strict-origin" : "no-referrer"}">
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'sha256-${scriptHash}'${analyticsScript}; style-src 'unsafe-inline'; img-src data:; connect-src ${connections}; font-src 'none'; base-uri 'none'; form-action 'none'">
...
<section id="mtv:document-tree" class="tree" aria-label="Markdown 文档树" tabindex="0">
