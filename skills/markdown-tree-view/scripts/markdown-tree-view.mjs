...
var package_default = {
  ...
  version: "0.1.0",
  description: "Convert a Markdown document into a self-contained offline HTML tree.",
  private: true,
  type: "module",
  license: "UNLICENSED",
  engines: { node: ">=24" },
  bin: { "markdown-tree-view": "bin/markdown-tree-view.mjs" },
  scripts: {
...
// src/render.mjs
var escapeHtml2 = (value) => String(value).replace(/[&<>"']/gu, (char) => ({
  "&": "&amp;",
  "<": "&lt;",
  ">": "&gt;",
  '"': "&quot;",
...
function transformLinks(tokens) {
  ...
  for (const token of tokens) {
    if (token.type === "link_open") {
      ...
      const link2 = stack.pop();
      if (link2 && !link2.allowed) {
        token.type = "blocked_link_close";
        token.content = link2.href;
      }
    }
  }
}
function parser() {
  const md = new MarkdownItCallable({ html: false, linkify: false, typographer: false });
  md.renderer.rules.image = (tokens, index) => {
    const token = tokens[index];
    const title = token.attrGet("title");
    return `<span class="image-placeholder resource-placeholder">[\u56FE\u7247\uFF1A${escapeHtml2(plainText(token.children) || "\u65E0\u66FF\u4EE3\u6587\u5B57")} \u2014 ${escapeHtml2(token.attrGet("src"))}${title ? ` \xB7 ${escapeHtml2(title)}` : ""}]</span>`;
  };
  md.renderer.rules.blocked_link_close = (tokens, index) => ` <span class="resource-address">(${escapeHtml2(tokens[index].content)})</span></span>`;
...
function parseDocument(markdown) {
  ...
  const stack = [root];
  const nodes = [];
  const usedIds = /* @__PURE__ */ new Set(["document-tree", "toggle-all"]);
  for (let i = 0; i < tokens.length; i++) {
    const token = tokens[i];
    ...
    const node = {
      ...
      line: token.map[0] + 1,
      endLine: token.map[1],
      title: plainText(inline2.children),
      titleHtml: md.renderer.renderInline(inline2.children, md.options, env),
      tokens: [],
      children: []
    };
...
function renderDocument(markdown, { sourceName = "document.md" } = {}) {
  ...
  const renderBody = (tokens, className) => tokens.length ? `<div class="markdown ${className}" data-source-line="${tokens.find((token) => token.map)?.map[0] + 1 || 1}">${md.renderer.render(tokens, md.options, env)}</div>` : "";
...
<summary class="tree-row" tabindex="0"><span class="chevron" aria-hidden="true"></span><span class="heading-label" id="${escapeHtml2(node.alias)}">${node.titleHtml || '<span class="empty-heading">\uFF08\u7A7A\u6807\u9898\uFF09</span>'}</span><span class="source-location" title="\u6E90\u6587\u4EF6\u7B2C ${node.line} \u884C">L${node.line}</span></summary>
