...
function parseDocument(markdown) {
  ...
  const stack = [root];
  const nodes = [];
  const usedIds = /* @__PURE__ */ new Set(["document-tree", "toggle-all", "theme-toggle", "help-toggle", "reading-help"]);
  for (let i = 0; i < tokens.length; i++) {
    const token = tokens[i];
